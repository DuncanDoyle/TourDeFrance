...
function usage() {
    ...
    echo
    echo "Example:"
    echo " $0 setup tdf --project-suffix s40d"
    echo
    echo "COMMANDS:"
    ...
    echo
    echo "DEMOS:"
    echo "   tdf	              Tour de France application."
    echo
    echo "OPTIONS:"
...
PRJ=("tdf-$PRJ_SUFFIX" "Tour De France" "Tour De France")
...
function import_secrets_and_service_account() {
  ...
  oc process -f https://raw.githubusercontent.com/jboss-container-images/rhpam-7-openshift-image/$OPENSHIFT_PAM7_TEMPLATES_TAG/example-app-secret-template.yaml -p SECRET_NAME=eap7-app-secret | oc create -f -
  oc create serviceaccount eap-service-account
  oc secrets link --for=mount eap-service-account eap7-app-secret
}

function create_application() {
  echo_header "Creating Tour De France application config."
  ...
  oc new-app --template=eap71-postgresql-persistent-s2i \
			-p APPLICATION_NAME="$ARG_DEMO" \
			-p IMAGE_STREAM_NAMESPACE="$IMAGE_STREAM_NAMESPACE" \
			-p SOURCE_REPOSITORY_URL="https://github.com/DuncanDoyle/TourDeFrance.git" \
			-p SOURCE_REPOSITORY_REF="development" \
			-p CONTEXT_DIR="" \
			-p DB_JNDI="java:jboss/datasources/PostgreSQLDS" \
			-p DB_DATABASE="tdf" 
