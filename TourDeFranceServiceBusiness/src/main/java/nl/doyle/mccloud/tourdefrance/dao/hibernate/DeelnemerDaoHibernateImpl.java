...
public class DeelnemerDaoHibernateImpl extends HibernateDaoSupport implements DeelnemerDao {
	 ...
	 * 
	 * Collection deelnemers = (Collection) getHibernateTemplate().execute(new HibernateCallback() { public Object doInHibernate(Session
	 * session) throws HibernateException { Query query = session.createQuery( "from Deelnemer deelnemer where deelnemer.nummer=?");
	 * query.setInteger(0, deelnemerNummer); return query.list(); } });
	 */
	/*
	 * Er kan hoogstens 1 deelnemer zijn, we zoeken immers met de primary key Haal deze deelnemer uit de collection en geef 'm terug
	 */
	/*
	 * Deelnemer returnDeelnemer = null; Iterator colIterator = deelnemers.iterator(); if (colIterator.hasNext()) { returnDeelnemer =
	 * (Deelnemer) colIterator.next(); } return returnDeelnemer; }
	 */

	@SuppressWarnings("unchecked")
	public Deelnemer loadDeelnemer(final int deelnemerNummer) {
		List<Deelnemer> result = getHibernateTemplate().find("from Deelnemer deelnemer where deelnemer.nummer=?",
				new Integer(deelnemerNummer));
	...
	public void saveDeelnemerDataExcludingForeignKeys(final int nummer, final String voornaam, final String achternaam, final String email,
		...
		final String hql = "update Deelnemer set voornaam = :newVoornaam, " + "achternaam = :newAchternaam, " + "email = :newEmail, "
				+ "rekeningnummer = :newRekeningnummer " + "where nummer = :deelnemerNummer";

		int rowCount = ((Integer) getHibernateTemplate().execute(new HibernateCallback() {
	...
	public Deelnemer loadDeelnemerHavingRenner(final Renner renner) {
		...
		Deelnemer deelnemer = (Deelnemer) getHibernateTemplate().execute(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException {
				Query query = session.createQuery(hql);
