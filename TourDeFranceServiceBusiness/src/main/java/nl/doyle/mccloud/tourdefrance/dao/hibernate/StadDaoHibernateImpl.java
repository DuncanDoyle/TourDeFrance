...
public class StadDaoHibernateImpl extends HibernateDaoSupport implements StadDao {
	...
	public Stad loadStad(int id) {
		List result = getHibernateTemplate().find("from Stad stad where stad.id=?", new Integer(id));
	...
	public List<Stad> loadStad(final String stad, final String land) {
		List<Stad> steden = 
			(List<Stad>) getHibernateTemplate().execute(new HibernateCallback() {
				public Object doInHibernate(Session session) throws HibernateException {
					Query query = session.createQuery(
					"from Stad stad WHERE stad.stad=? AND stad.land=?");
